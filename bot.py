...
from config import config, ROOT_DIR, SECRET_KEY, PAYMENT_TOKEN
import json
import re
import database
...
db = database.DBHandler(dialect=os.environ['DIALECT'],
                        user=os.environ['USER'], password=os.environ['PASSWORD'],
                        host=os.environ['HOST'], dbname='das8ek7meriqs4')
db.create_internal_tables()


@bot.message_handler(commands=['inttabl343434'])
def internal_tables_heroku(message):
    db.create_internal_tables()
...
@bot.message_handler(regexp='addproduct'+(SECRET_KEY)+r". \w+")
...
def add_to_cart(query):
    ...
    if prod_id is not None:
        db.add_products_to_cart(user_id, prod_id)
        return prod_id
...
def add_order_callback(query):
    try:
        ...
        text = 'Ваш кошик: \n' + make_cart_text(query.message.chat.id)
